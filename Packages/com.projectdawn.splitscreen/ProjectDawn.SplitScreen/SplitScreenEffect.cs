...
namespace ProjectDawn.SplitScreen
{
    ...
    [ExecuteAlways]
    [HelpURL("https://lukaschod.github.io/adaptive-split-screen-docs/manual/split-screen-effect.html")]
    public class SplitScreenEffect : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        /// Adds to the command buffer commands needed to render split screen.
        /// </summary>
        public void UpdateCommandBuffer(RasterCommandBuffer cmd, UnityEngine.Rendering.RenderGraphModule.TextureHandle[] handles)
        {
            if (!m_ScreenRegions.IsCreated)
                return;
            ...
            using (Markers.UpdateCommandBuffer.Auto())
            {
                ...
                for (int i = 0; i < Screens.Count; ++i)
                {
                    cmd.SetGlobalTexture("_SplitScreenTexture", handles[i]);
                    cmd.DrawMesh(Screens[i].Mesh, Matrix4x4.identity, Material, 0, 0);
                }
        ...
        void OnEnable()
        {
            ...
            if (QualitySettings.renderPipeline == null && GraphicsSettings.defaultRenderPipeline == null)
            {
